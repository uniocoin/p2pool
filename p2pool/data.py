...
import p2pool
from p2pool.unio import data as unio_data, script, sha256
...
class WeightsSkipList(forest.TrackerSkipList):
    ...
    def get_delta(self, element):
        ...
        att = unio_data.target_to_average_attempts(share.target)
...
class OkayTracker(forest.Tracker):
    def __init__(self, net):
        forest.Tracker.__init__(self, delta_type=forest.get_attributedelta_type(dict(forest.AttributeDelta.attrs,
            work=lambda share: unio_data.target_to_average_attempts(share.target),
            min_work=lambda share: unio_data.target_to_average_attempts(share.max_target),
        )))
        self.net = net
    ...
    def think(self, block_rel_height_func, previous_block, bits, known_txs):
        ...
        if p2pool.DEBUG:
            print 'Desire %i shares. Cutoff: %s old diff>%.2f' % (len(desired), math.format_dt(time.time() - timestamp_cutoff), unio_data.target_to_difficulty(target_cutoff))
            for peer_addr, hash, ts, targ in desired:
                print '   ', None if peer_addr is None else '%s:%i' % peer_addr, format_hash(hash), math.format_dt(time.time() - ts), unio_data.target_to_difficulty(targ), ts >= timestamp_cutoff, targ <= target_cutoff
...
def get_stale_counts(tracker, share_hash, lookbehind, rates=False):
    ...
    for share in tracker.get_chain(share_hash, lookbehind - 1):
        res['good'] = res.get('good', 0) + unio_data.target_to_average_attempts(share.target)
        s = share.share_data['stale_info']
        if s is not None:
            res[s] = res.get(s, 0) + unio_data.target_to_average_attempts(share.target)
    if rates:
...
def get_expected_payouts(tracker, best_share_hash, block_target, subsidy, net):
    weights, total_weight, donation_weight = tracker.get_cumulative_weights(best_share_hash, min(tracker.get_height(best_share_hash), net.REAL_CHAIN_LENGTH), 65535*net.SPREAD*unio_data.target_to_average_attempts(block_target))
...
def get_desired_version_counts(tracker, best_share_hash, dist):
    ...
    for share in tracker.get_chain(best_share_hash, dist):
        res[share.desired_version] = res.get(share.desired_version, 0) + unio_data.target_to_average_attempts(share.target)
    return res

def get_warnings(tracker, best_share, net, uniod_getinfo, uniod_work_value):
    res = []
    ...
    if uniod_getinfo['errors'] != '':
        if 'This is a pre-release test build' not in uniod_getinfo['errors']:
            res.append('(from uniod) %s' % (uniod_getinfo['errors'],))
    
    version_warning = getattr(net, 'VERSION_WARNING', lambda v: None)(uniod_getinfo['version'])
    if version_warning is not None:
        res.append(version_warning)
    
    if time.time() > uniod_work_value['last_update'] + 60:
        res.append('''LOST CONTACT WITH UNIOD for %s! Check that it isn't frozen or dead!''' % (math.format_dt(time.time() - uniod_work_value['last_update']),))
